...
* **Estrutura de Declaração e Glossário (INVEST - Pequena & Negociável):** O item deve seguir o formato padrão orientado a valor:
...
* **Validação Técnica e Cobertura de Testes:** O código deve passar por testes unitários e de integração automatizados. A entrega exige uma cobertura mínima de **80% geral do código** (linhas e ramificações). Em módulos críticos de segurança (como geração de cadeia de hash SHA-256 e assinatura digital ICP-Brasil), a cobertura obrigatória é de **95%**, cobrindo cenários de sucesso e falha simulada.
* **Revisão Colaborativa (Code Review):** O código deve ser revisado e aprovado no GitHub por pelo menos um par desenvolvedor. A revisão garante o alinhamento com os padrões de arquitetura do projeto, a segurança no tratamento de dados sensíveis e a ausência de vulnerabilidades.
